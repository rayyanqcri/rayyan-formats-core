  ...
  module Plugins
    class Base
      ...
      def method_missing(method_sym, *arguments, &block)
        if method_sym.in? [:title, :extension, :description, :detect, :parse, :is_core?]
      ...
      class << self  
        ...
        def parse(*arguments, &block)
          if arguments.length == 0
            @format_parse_block = block
          else
            Rails.logger.debug "Inside #{self.title} parser for file: #{arguments[1] rescue ''}"
            @format_parse_block.call(*arguments, &block)
          end
        end

        def formats=(list)
          @@text_format = ArticleImportFormatPlainText.new
          @@csv_format = ArticleImportFormatCSV.new
          @@formats = list.reject{|klass|
            klass.in? [ArticleImportFormatPlainText, ArticleImportFormatCSV]
          }.map(&:new) << @@text_format << @@csv_format
        ...
        def process(source, &block)
          ...
          format.parse(file.read, filename) do |mArticle, total, is_new|
            article = nil

            if is_new.nil?
              mArticle.source = source
              article, is_new = Article.persist_or_merge_with_existing(mArticle)
            else
              article = mArticle
            end

            block.call(article, is_new, total) if block
          end
